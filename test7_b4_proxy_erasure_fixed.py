"""测试7: B4 黑盒代理模型盲擦除攻击 (NAACL 2025) - 固定版本"""

# ============================================================================
# 关键修复：在导入 test_common 之前，强制环境使用 OPT
# ============================================================================
import os
import sys

# 方法1：修改 model_selector 的选择逻辑
def force_opt_model():
    """临时覆盖 model_selector 的返回值"""
    import model_selector as ms
    original_func = ms.get_model_for_test
    def patched_func(*args, **kwargs):
        return "facebook/opt-1.3b"
    ms.get_model_for_test = patched_func
    return original_func

# 在导入 test_common 之前先 patch
sys.path.insert(0, os.path.dirname(__file__))
_original_selector = force_opt_model()

# 现在才导入 test_common（此时会使用 OPT 而不是 Gemma2）
from test_common import *
from test_common import _get_local_model_path
import time

print_test_header("B4 黑盒代理模型盲擦除攻击 (NAACL 2025)")

# 确认使用的是 OPT
print(f">>> 检测器模型类型: {type(target_model).__name__}")
print(f">>> Tokenizer 类型: {type(detector_tokenizer).__name__}")

# 确保使用 OPT（不是 Gemma）
if 'Gemma' in type(target_model).__name__:
    print("⚠️  警告：仍在使用 Gemma，强制切换到 OPT...")
...
for algo in algorithms:
    ...
    for text in original_texts:
        z_before = detect_watermark(text, algo, detector_tokenizer, vocab_size, model=target_model, device=device)
    ...
    for text in attacked_texts:
        z_after = detect_watermark(text, algo, detector_tokenizer, vocab_size, model=target_model, device=device)
